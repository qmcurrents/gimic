...
!        write(fd, *) '   </CellData>'
    ...
    subroutine write_j2d_imagedata(fname, grid, pdata)
        character(*), intent(in) :: fname
        type(grid_t) :: grid
        real(DP), dimension(:,:,:,:) :: pdata
        ...
        write(fd,*) "npts(1)", npts(1)
        write(fd,*) "npts(2)", npts(2)
        write(fd,*) "npts(3)", npts(3)
        write(fd,*) "qmin", qmin 
        write(fd,*) "qmax", qmax
        write(fd,*) "step", step
        write(fd,*) "npts", npts
         ...
         do k=1,npts(3)
             do j=1,npts(2)
                 do i=1,npts(1)
                    ...
                    !end do
                    !l=l+1
                    rr=gridpoint(grid,i,j,k)*AU2A

                    !write(fd,'(6e14.6)') rr(1), rr(2), rr(3), &
                    !pdata(i,j,k,1), &
                    !pdata(i,j,k,2), pdata(i,j,k,3)
                    write(fd,'(4e20.9)') rr(1), rr(2), pdata(i,j,k,1), & 
                    &                    pdata(i,j,k,2)
                end do
            end do
        end do
