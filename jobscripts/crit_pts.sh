...
{ 
    ...
    # take a third point C from the plane that lies at (x0x, x0y, 1)
    # define vector a: C -> x0; vector b: C -> x1;
    # calculate their cross product to find the normal vector to the plane
...
dirname=${dir##*/} # pick the name of the current directory only
...
echo $x0 $x1 | normalPlane
echo

printf "\n# Normal to the clipping plane:\n# " >> $wrkdir/profile-points-paraview.dat
echo $x0 $x1 | normalPlane >> $wrkdir/profile-points-paraview.dat
