...
contains
	subroutine write_cubeplot(fname, grid, pdata, skip)
		...
		real(DP), dimension(3) :: origin, step
...
! flagged in grid.f90 and interpolated. grid.f90 should be reworked, big time.
		qrange=get_grid_range(grid,1)
		step(1)=(qrange(2)-qrange(1))/(npts(1)-1)
		origin(1)=qrange(1)

		qrange=get_grid_range(grid,2)
		step(2)=(qrange(2)-qrange(1))/(npts(2)-1)
		origin(2)=qrange(1)

		qrange=get_grid_range(grid,3)
		step(3)=(qrange(2)-qrange(1))/(npts(3)-1)
		origin(3)=qrange(1)
